...
_TOOL_STUBS = {
    ...
    "write_file": (
        "write_file",
        "path: str, content: str, cross_profile: bool = False, dry_run: bool = False",
        '"""Write content to a file (always overwrites unless dry_run=True). Returns dict with status. cross_profile=True opts out of the cross-Hermes-profile soft guard."""',
        '{"path": path, "content": content, "cross_profile": cross_profile, "dry_run": dry_run}',
    ),
    "search_files": (
        "search_files",
    ...
    "patch": (
        "patch",
        'path: str = None, old_string: str = None, new_string: str = None, replace_all: bool = False, mode: str = "replace", patch: str = None, cross_profile: bool = False, dry_run: bool = False',
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Hermes-profile soft guard; dry_run=True previews without writing."""',
        '{"path": path, "old_string": old_string, "new_string": new_string, "replace_all": replace_all, "mode": mode, "patch": patch, "cross_profile": cross_profile, "dry_run": dry_run}',
    ),
    "terminal": (
        "terminal",
